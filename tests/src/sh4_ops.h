...
#define ASSERT_CLEAN_REG(regNum)         \
	{                                    \
		ASSERT_EQ(r(regNum), REG_MAGIC); \
	}
...
protected:
	...
	u32& r(u32 regNum)
	{
		checkedRegs.insert(&ctx->r[regNum]);
		return ctx->r[regNum];
	}
	u32& gbr()
	{
		checkedRegs.insert(&ctx->gbr);
		return ctx->gbr;
	}
	...
	f32& fr(int regNum)
	{
		checkedRegs.insert((u32*)&ctx->fr[regNum]);
		return ctx->fr[regNum];
	}
