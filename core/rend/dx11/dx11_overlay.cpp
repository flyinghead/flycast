...
void DX11Overlay::draw(u32 width, u32 height, bool vmu, bool crosshair)
{
	...
	if (vmu)
	{
		...
		if (GetAsyncKeyState(VK_LBUTTON) & 0x8000) {
			if (!isDragging) {
				// Check if click is within any VMU
				for (int i = 0; i < 8; i++) {
					if (!vmu_lcd_status[i]) continue;

					float x = (i & 2) ? width - vmu_padding - vmu_width : vmu_padding;
					float y = (i & 4) ? height - vmu_padding - vmu_height : vmu_padding;
					x += dragOffsetX[i];
					y += dragOffsetY[i];

					if (currentMousePos.x >= x && currentMousePos.x <= x + vmu_width &&
						currentMousePos.y >= y && currentMousePos.y <= y + vmu_height) {
						isDragging = true;
						draggedVMU = i;
				...
				dragOffsetX[draggedVMU] += currentMousePos.x - lastMousePos.x;
				dragOffsetY[draggedVMU] += currentMousePos.y - lastMousePos.y;
				lastMousePos = currentMousePos;
			}
		}
		else {
		...
		for (size_t i = 0; i < vmuTextures.size(); i++)
		{
			...
			// Keep VMU within screen bounds
			x = std::max(0.f, std::min(x, width - vmu_width));
			y = std::max(0.f, std::min(y, height - vmu_height));
#endif
			D3D11_VIEWPORT vp{};
			vp.TopLeftX = x;
			vp.TopLeftY = y;
