...
static bool FastDecode(uint16_t raw, uint32_t pc, Instr &ins, Block &blk)
{
    ...
    // ADDC Rm,Rn (0x3nmE) - Add with carry - FIXED: was incorrectly 0x000E
    ...
    // MOV.L @(R0, Rm), Rn (0x0nm2) - Alternative pattern for R0-based load
    else if ((raw & 0xF00F) == 0x0002)
    {
        uint8_t n = (raw >> 8) & 0xF;
        uint8_t m = (raw >> 4) & 0xF;
        ins.op = Op::LOAD32_R0;
        ...
        ins.src1.isImm = false; ins.src1.reg = m; // Rm (index register)
        blk.pcNext = pc + 2;
        DEBUG_LOG(SH4, "FastDecode: MOV.L @(R0,R%u),R%u (alt 0x%04X)", m, n, raw);
        return true;
    }
    // MOV.L Rm, @(R0, Rn) (0x0nm6) - Store Rm to R0+Rn
    ...
    // MOV.W Rm, @(R0, Rn) (0x0nm5) - Store word to R0+Rn
    ...
    // MOV.B Rm, @(R0, Rn) (0x0nm4) - Store byte to R0+Rn
...
// #endif
    ...
    // BT disp8 (0x8900-0x89FF) – branch if T==1, no delay slot
        ...
        {
            uint8_t n = (raw >> 4) & 0xF;  // FIX: Extract Rn from bits 7-4, not 11-8
            uint8_t disp = raw & 0xF;
            ins.op = Op::STORE8;
            ins.src1.isImm = false; ins.src1.reg = 0; // R0
    ...
    // === ADDITIONAL FASTDECODE PATTERNS FOR TOP MISSING OPCODES ===

    // MOVA @(disp,PC),R0 (0xC7xx) - PC-relative address calculation
    else if ((raw & 0xFF00) == 0xC700)
    {
        ...
        ins.op = Op::MOV_IMM;
        ins.dst.isImm = false; ins.dst.reg = 0; // R0
        ins.src1.isImm = true; ins.src1.imm = effective_address;
        blk.pcNext = pc + 2;
        DEBUG_LOG(SH4, "FastDecode: MOVA @(0x%02X,PC),R0 -> 0x%08X (0x%04X)", disp, effective_address, raw);
        return true;
    }
    // MOV.W @(disp,GBR),R0 (0xC5xx) - GBR-relative word load
    else if ((raw & 0xFF00) == 0xC500)
    {
        uint8_t disp = raw & 0xFF;
        ins.op = Op::LOAD16_GBR;
        ins.dst.isImm = false; ins.dst.reg = 0; // R0
        ins.extra = disp; // raw displacement - executor will shift it
        blk.pcNext = pc + 2;
        DEBUG_LOG(SH4, "FastDecode: MOV.W @(0x%02X,GBR),R0 (0x%04X)", disp, raw);
        return true;
    }
    // MOV.B @(disp,GBR),R0 (0xC4xx) - GBR-relative byte load
    else if ((raw & 0xFF00) == 0xC400)
    {
        uint8_t disp = raw & 0xFF;
        ins.op = Op::LOAD8_GBR;
        ins.dst.isImm = false; ins.dst.reg = 0; // R0
        ins.extra = disp; // raw displacement for bytes
        blk.pcNext = pc + 2;
        DEBUG_LOG(SH4, "FastDecode: MOV.B @(0x%02X,GBR),R0 (0x%04X)", disp, raw);
        return true;
    }
    // FMOV.S FRm,@-Rn (0xFnm7) - Floating point pre-decrement store
    else if ((raw & 0xF00F) == 0xF007)
    {
        uint8_t n = (raw >> 8) & 0xF;
        uint8_t m = (raw >> 4) & 0xF;
        ins.op = Op::FMOV_STORE_PREDEC;
        ins.dst.isImm = false; ins.dst.reg = n; // Rn (address register)
        ins.src1.isImm = false; ins.src1.reg = m; ins.src1.type = RegType::FGR; // FRm (value)
        blk.pcNext = pc + 2;
        DEBUG_LOG(SH4, "FastDecode: FMOV.S FR%u,@-R%u (0x%04X)", m, n, raw);
        return true;
    }
    // MOV.L Rm,@(R0,Rn) (0x0nm6) - already implemented above
    // MOV.L @(R0,Rm),Rn (0x0nmE) - already implemented above

    // MOV.B Rm,@Rn (0x2nm0) - Store byte
    else if ((raw & 0xF00F) == 0x2000)
    {
        uint8_t n = (raw >> 8) & 0xF;
        uint8_t m = (raw >> 4) & 0xF;
        ins.op = Op::STORE8;
        ins.src1.isImm = false; ins.src1.reg = m; // Rm (value)
        ins.src2.isImm = false; ins.src2.reg = n; // Rn (address)
        ins.extra = 0; // no displacement
        blk.pcNext = pc + 2;
        DEBUG_LOG(SH4, "FastDecode: MOV.B R%u,@R%u (0x%04X)", m, n, raw);
        return true;
    }
    // MOV.W @(R0,Rm),Rn (0x0nm1) - Load word with R0 offset
    else if ((raw & 0xF00F) == 0x0001)
    {
        uint8_t n = (raw >> 8) & 0xF;
        uint8_t m = (raw >> 4) & 0xF;
        ins.op = Op::LOAD16_R0;
    ...
    // MOV.B @(R0,Rm),Rn (0x0nm0) - Load byte with R0 offset
    else if ((raw & 0xF00F) == 0x0000)
    {
        uint8_t n = (raw >> 8) & 0xF;
        uint8_t m = (raw >> 4) & 0xF;
    ...
    else if ((raw & 0xF000) == 0x1000)
    {
        ...
        ins.op = Op::STORE32;
        ins.src1.isImm = false; ins.src1.reg = m; // Rm (value)
        ins.src2.isImm = false; ins.src2.reg = n; // Rn (base address)
        ins.extra = disp * 4; // byte displacement
        blk.pcNext = pc + 2;
        DEBUG_LOG(SH4, "FastDecode: MOV.L R%u,@(%u,R%u) (0x%04X)", m, disp*4, n, raw);
        return true;
    }
    // MOV Rm,Rn (0x6nm3) - already implemented above
