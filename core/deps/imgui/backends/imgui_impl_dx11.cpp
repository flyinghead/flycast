...
/* Flycast: Use ImTextureDX11 instead
// DirectX11 data
...
struct ImGui_ImplDX11_Data
{
    ...
    ID3D11SamplerState*         pTexSamplerLinearBorder;
    ...
    for (const ImDrawList* draw_list : draw_data->CmdLists)
    {
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                if((ImTextureID)tex != ImGui::GetIO().Fonts->TexID.GetTexID())
                {
                    if (tex->pointSampling)
                        device->PSSetSamplers(0, 1, &bd->pTexSamplerNearest);
                    else
                        device->PSSetSamplers(0, 1, &bd->pTexSamplerLinearBorder);
                }
                else
...
{
    if (ImTextureDX11* backend_tex = (ImTextureDX11*)tex->BackendUserData)
    {
        IM_ASSERT(backend_tex == (ImTextureDX11*)tex->TexID);
        backend_tex->shaderResourceView->Release();
...
void ImGui_ImplDX11_UpdateTexture(ImTextureData* tex)
{
    ...
    if (tex->Status == ImTextureStatus_WantCreate)
    {
        ...
        ImTextureDX11* backend_tex = IM_NEW(ImTextureDX11)();
        ...
        bd->pd3dDevice->CreateShaderResourceView(backend_tex->pTexture, &srvDesc, &backend_tex->shaderResourceView);
        IM_ASSERT(backend_tex->shaderResourceView != nullptr && "Backend failed to create texture!");

        // Store identifiers
        tex->SetTexID((ImTextureID)backend_tex);
    ...
    else if (tex->Status == ImTextureStatus_WantUpdates)
    {
        ...
        ImTextureDX11* backend_tex = (ImTextureDX11*)tex->BackendUserData;
        IM_ASSERT(backend_tex == (ImTextureDX11*)tex->TexID);
...
bool    ImGui_ImplDX11_CreateDeviceObjects()
{
    ...
    // (Bilinear sampling is required by default. Set 'io.Fonts->Flags |= ImFontAtlasFlags_NoBakedLines' or 'style.AntiAliasedLinesUseTex = false' to allow point/nearest sampling)
    {
        ...
        bd->pd3dDevice->CreateSamplerState(&desc, &bd->pTexSamplerLinearBorder);
...
void    ImGui_ImplDX11_InvalidateDeviceObjects()
{
    ...
    if (bd->pTexSamplerLinearBorder){ bd->pTexSamplerLinearBorder->Release(); bd->pTexSamplerLinearBorder = nullptr; }
